...
# Cancels older runs of the same workflow on the same branch
# Allows different workflows and branches to remain in queue
concurrency:
...
jobs:
  job-name:
    ...
    # Single runner executes jobs one at a time from queue
```
...
1. **Smart Concurrency Control:**
   ...
   - Single runner executes one job at a time
